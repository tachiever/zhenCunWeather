        ...
        case JHLineChartQuadrantTypeFirstQuardrant:{
            ...
            if (_xLineDataArr.count>0) {
                ...
                for (NSInteger i = 0; i<_xLineDataArr.count;i++ ) {
                    ...
                    [self drawText:[NSString stringWithFormat:@"%@",_xLineDataArr[i]] andContext:context atPoint:P_M(p.x-len/2, p.y-3) WithColor:_xAndYNumberColor andFontSize:self.xDescTextFontSize];
                }
...
- (void)drawPositionLineWithContext:(CGContextRef)context{
    ...
    for (NSInteger m = 0;m<_valueArr.count;m++) {
        ...
        for (NSInteger i = 0 ;i<arr.count;i++ ) {
            ...
            if (p.y!=0) {
                UIColor *pointNumberColor = (_pointNumberColorArr.count == _valueArr.count?(_pointNumberColorArr[m]):([UIColor whiteColor]));

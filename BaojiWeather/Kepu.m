...
@interface Kepu ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>{
...
- (void)createTableView{
    ...
    NSArray *arr=@[@"气象词典",@"气象百科",@"灾害防御"];
    for (int i=0; i<arr.count; i++) {
        UIButton * searchBtn=[[UIButton alloc]initWithFrame:CGRectMake(self.frame.size.width/3*i,0,self.frame.size.width/3,39)];
        searchBtn.tag=i;
    ...
    _signView=[[UIView alloc]initWithFrame:CGRectMake(0, 37, self.frame.size.width/3, 3)];
...
-(void)loadSearchData:(NSDictionary *)dic KeyWords:(NSString *)ky{
    ...
    [dict setObject:@"气象词典" forKey:@"type"];
    AFHTTPRequestOperationManager *manger = [AFHTTPRequestOperationManager manager];
    manger.responseSerializer = [AFHTTPResponseSerializer serializer];
    [manger POST:AtmoSever parameters:dict success:^(AFHTTPRequestOperation * operation, id responseObject) {
        NSDictionary *dict = [NSJSONSerialization JSONObjectWithData:responseObject options:NSJSONReadingMutableContainers error:nil];
...
- (void)signViewMove:(NSInteger)inte{
    CGRect fram= CGRectMake(self.frame.size.width/3*inte, 37, self.frame.size.width/3, 3);
